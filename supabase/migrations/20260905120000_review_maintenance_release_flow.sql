-- PROPOSTA PARA REVISÃO: fechamento transacional de manutenção.
-- Não executar automaticamente. Revisar no Supabase antes de aplicar.
  ...
  if not exists (
    select 1
     ...
     where schemaname = 'public'
       and indexname = 'maintenance_releases_record_unique_idx'
  ) then
    create unique index maintenance_releases_record_unique_idx
      on public.maintenance_releases (maintenance_record_id)
...
begin
  ...
  if p_action not in ('start', 'complete_and_release') then
    raise exception using
      errcode = '22023',
